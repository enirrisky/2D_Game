...
//#include <GL\glew.h>        // gl ext library
...
void drawCallback()
{
	...
	ui.callBack(&ui, ui.p); //here's were we are calling the callback defined in main
...
void keyDownCallback(int key, int x, int y)
{
	...
	Interface ui;
	ui.keySpecialEvent(key, true /*fDown*/);
}
...
void keyUpCallback(int key, int x, int y)
{
	...
	// so we are actually getting the same version as in the constructor.
	Interface ui;
	ui.keySpecialEvent(key, false /*fDown*/);
}
 ...
 *           fDown   down or brown
 ****************************************************************/
void Interface::keyEvent(unsigned char key, bool fDown)
{
	switch (key)
	{
	case ' ':
 ...
 *           fDown   down or brown
 ****************************************************************/
void Interface::keySpecialEvent(int key, bool fDown)
{
	switch (key)
	{
	case GLUT_KEY_DOWN:
...
void Interface::initialize(int argc, char ** argv, const char * title, Point topLeft, Point bottomRight)
{
	...
	initialized = true; // So our static Interface class doesn't get intialized twice.
